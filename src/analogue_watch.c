...
#define Key_HandColor 3
#define Key_HourColor 4
#define Key_DivisionColor 5

static int s_interval_second = 1, s_hourDigit_show = 1;
static GColor s_background_color, s_shadow_color;
static GColor s_hand_color;
static GColor s_hour_color;
static GColor s_division_color;
...
static const int DivisionLength = 123;
#define HandLength 110
static const GPoint ShadowOffset = {
  .x = 2, .y = 3
};
...
inline char char_minus(const char a, const char b) {
  return (a > b) ? a - b : 0;   
}

// drawing

static void prepare_drawing_hand_division(Layer * const layer, 
                      GContext * const ctx,
										  const int hour24, const int minute) {
...
#ifdef PBL_COLOR
  GPoint shadow_shift = {.x = s_hand_shift.x + ShadowOffset.x,
                         .y = s_hand_shift.y + ShadowOffset.y};
  graphics_context_set_stroke_width(ctx, 3);
  gpath_move_to(s_hand_path_ptr, shadow_shift);
  graphics_context_set_stroke_color(ctx, s_shadow_color);
...
static void draw_division(Layer * const layer, GContext * const ctx,
						  const int minute, const bool filled) {
	graphics_context_set_stroke_color(ctx, s_division_color);
	graphics_context_set_fill_color(ctx, s_division_color);
	...
	const int radius = (minute % 5 == 0) ? 6 : 3;
#ifdef PBL_COLOR
...
static void draw_hand_filled(Layer * const layer, GContext * const ctx, const int second) {
	...
	graphics_context_set_fill_color(ctx, s_hand_color);
...
#ifdef PBL_COLOR
  const int sh_cx = cx + ShadowOffset.x;
  const int sh_cy = cy + ShadowOffset.y;
  graphics_context_set_fill_color(ctx, s_shadow_color);
  graphics_fill_circle(ctx, GPoint(sh_cx, sh_cy), radius);
#endif
	graphics_context_set_fill_color(ctx, s_hand_color);
	...
	while (t != NULL) {
		switch (t->key) {
		  case Key_BackgroundColor:
			APP_LOG(APP_LOG_LEVEL_INFO, "bgcolor_color %d", (int)t->value->int32);
			persist_write_int(Key_BackgroundColor, (int)t->value->int32);
			break;
      case Key_HandColor:
			APP_LOG(APP_LOG_LEVEL_INFO, "hand_color %d", (int)t->value->int32);
			persist_write_int(Key_HandColor, (int)t->value->int32);
			break;
      case Key_HourColor:
			APP_LOG(APP_LOG_LEVEL_INFO, "hour_white %d", (int)t->value->int32);
			persist_write_int(Key_HourColor, (int)t->value->int32);
			break;
      case Key_DivisionColor:
			APP_LOG(APP_LOG_LEVEL_INFO, "division_white %d", (int)t->value->int32);
			persist_write_int(Key_DivisionColor, (int)t->value->int32);
			break;
...
static void read_persist_values(void) {
  const int bg_color = persist_read_int(Key_BackgroundColor);
#ifdef PBL_BW
  if (bg_color == 255) {  // white
		s_background_color = s_hour_color = GColorWhite;
		s_hand_color = s_division_color = GColorBlack;
  } else { // black
		s_background_color = s_hour_color = GColorBlack;
		s_hand_color = s_division_color = GColorWhite;
  }
#else
  s_background_color = (GColor8){.argb = bg_color};
  s_shadow_color = (GColor8){.a = 0b11,
    .r = char_minus(s_background_color.r, 1),
    .g = char_minus(s_background_color.g, 1),
    .b = char_minus(s_background_color.b, 1)};
  s_hand_color = (GColor8){.argb = persist_read_int(Key_HandColor)};
  s_hour_color = (GColor8){.argb = persist_read_int(Key_HourColor)};
  s_division_color = (GColor8){.argb = persist_read_int(Key_DivisionColor)};
#endif
...
static void init(void) {
	...
	s_hand_color = GColorClear;
	s_hour_color = s_division_color = GColorClear;
